...
export default function TodoItem({children, id, text, priority, finishBy, status, changeTodoStatus, setTodoList }: TodoItemProps) {
    ...
    return (
    <div id={id} className="todo flex justify-between"><p className="priority" id={`priority${id}`}>{priority}</p><p className={`todo-text ${status}`} onClick={()=>{changeTodoStatus(id, setTodoList)}}> {text} by {finishDate} </p>{children}</div>
  )
}
